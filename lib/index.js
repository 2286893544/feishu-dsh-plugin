...
function toolsOf(client) {
  return [
    defineTool({
      ...
      parameters: {},
      async execute() {
        return { chats: await client.listChats() };
      },
    }),
    defineTool({
      ...
      async execute(args) {
        return await client.sendText(args.chat_id, args.text);
      },
    }),
    defineTool({
      name: "feishu_read_chat_history",
      description: "Read recent messages of a Feishu chat the bot is in (chronological). Requires the app scope for reading group history.",
      parameters: {
      ...
      async execute(args) {
        return { messages: await client.readChatHistory(args.chat_id, args.limit) };
      },
    }),
    defineTool({
      name: "feishu_create_document",
      description: "Create a new Feishu cloud document (docx) with a title and optional paragraphs, and return its document_id and share URL when a tenantDomain is configured.",
      parameters: {
        title: { type: "string", required: true, description: "Document title." },
        paragraphs: {
          type: "array",
          items: { type: "string" },
          description: "Optional list of plain-text paragraphs to write into the document.",
        },
      },
      async execute(args) {
        return await client.createDocument(args.title, args.paragraphs || []);
      },
    }),
    defineTool({
      ...
      async execute(args) {
        return await client.readDocument(args.document_id);
      },
    }),
    defineTool({
      ...
      async execute(args) {
        return await client.appendBlocks(args.document_id, args.blocks || []);
      },
    }),
    defineTool({
      name: "feishu_grant_document_access",
      description: "Grant a Feishu user (open_id) full access to a document or bitable the app owns, so it appears in their drive.",
      parameters: {
      ...
      async execute(args) {
        return await client.grantAccess(args.token, args.type, args.open_id);
      },
    }),
  ];
}
...
function resolveCredentials(config = {}) {
  ...
  if (!appId || !appSecret) {
    throw new Error(
      ...
      "(optionally tenantDomain, defaultChatId) in the profile patch, or set the " +

...
function resolveCredentials(config = {}) {
  ...
  if (!appId || !appSecret) {
    throw new Error(
      "feishu-bridge: missing Feishu credentials. Open Settings → Plugins → Plugin configuration " +
      "and set appId / appSecret (optionally tenantDomain, defaultChatId), or set the " +
...
function apply(ctx, config = {}) {
  ...
  for (const tool of toolsOf(client, config)) ctx.tools.register(tool);
